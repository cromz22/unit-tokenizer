...
from unit_tokenizer import NaivePackBitsTokenizer


def test_encode():
    tokenizer = NaivePackBitsTokenizer()
    encoded = tokenizer._encode([0, 0, 0, 0, 1, 1, 2, 2, 2, 2, 2, 2, 3, 4, 5, 6])
    assert encoded == [4, 100, 2, 101, 6, 102, -4, 103, 104, 105, 106]


def test_decode():
    tokenizer = NaivePackBitsTokenizer()
    decoded = tokenizer._decode([4, 100, 2, 101, 6, 102, -4, 103, 104, 105, 106])
...
def test_batch_decode():
    tokenizer = NaivePackBitsTokenizer()
    decoded = tokenizer.decode([[4, 100, 2, 101], [6, 102, -4, 103, 104, 105, 106]])
    assert decoded == [[0, 0, 0, 0, 1, 1], [2, 2, 2, 2, 2, 2, 3, 4, 5, 6]]


def test_max_run_length_encode():
    tokenizer = NaivePackBitsTokenizer(max_run_length=9, shift=10)
    encoded = tokenizer.encode(
        [[0, 1, 2, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3], [3, 3, 3]]
    )
    assert encoded == [[-3, 10, 11, 12, 9, 13, 3, 13], [3, 13]]


def test_max_run_length_decode():
    tokenizer = NaivePackBitsTokenizer(max_run_length=9, shift=10)
    decoded = tokenizer.decode([[-3, 10, 11, 12, 9, 13, 3, 13], [3, 13]])
